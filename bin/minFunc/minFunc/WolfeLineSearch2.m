function [t,f_new,g_new,funEvals,H] = WolfeLineSearch2(...
    x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,maxLS,progTol,debug,doPlot,saveHessianComp,funObj,updateFuncObj, updateFuncParams,varargin)
%
...
xn = updateFuncObj(x, t, d, updateFuncParams);
if nargout == 5
    [f_new,g_new,H] = funObj(xn,varargin{:});
else
    [f_new,g_new] = funObj(xn,varargin{:});
end
funEvals = 1;
gtd_new = g_new'*d;
...
while LSiter < maxLS
    ...
    if ~isLegal(f_new) || ~isLegal(g_new)
        ...
        % Do Armijo
        if nargout == 5
            [t,x_new,f_new,g_new,armijoFunEvals,H] = ArmijoBacktrack2(...
                x,t,d,f,f,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,saveHessianComp,...
                funObj,updateFuncObj, updateFuncParams, varargin{:});
        else
            [t,x_new,f_new,g_new,armijoFunEvals] = ArmijoBacktrack2(...
                x,t,d,f,f,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,saveHessianComp,...
                funObj,updateFuncObj, updateFuncParams, varargin{:});
        end
