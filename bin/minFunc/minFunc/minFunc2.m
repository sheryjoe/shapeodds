function [x,f,exitflag,output] = minFunc2(funObj,x0,options,varargin)
% this function also takes a function handle for performing the update
% scheme
...
if ~isfield(options, 'updateFuncObj')
    fprintf('minFunc2 is only used with a user-defined update function ...');
    return;
end

updateFuncObj = options.updateFuncObj;
if isfield(options, 'updateFuncParams')
    updateFuncParams = options.updateFuncParams;
else
    updateFuncParams = [];
end

if ~isfield(options, 'useFixedTimeStep')
    useFixedTimeStep = 0;
else
     useFixedTimeStep = options.useFixedTimeStep;
%     if useFixedTimeStep
%         if ~isfield(options, 'useFixedTimeStep')
%             fprintf('useFixedTimeStep was enabled without specifying fixedTimeStep field');
%         else
...
for i = 1:maxIter
    ...
    if ~useFixedTimeStep
        ...
        if LS_type == 0 % Use Armijo Bactracking
            ...
            if computeHessian
                [t,x,f,g,LSfunEvals,H] = ArmijoBacktrack2(x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,LS_saveHessianComp,funObj,updateFuncObj, updateFuncParams,varargin{:});
            else
                [t,x,f,g,LSfunEvals] = ArmijoBacktrack2(x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,1,funObj,updateFuncObj, updateFuncParams,varargin{:});
            end
        ...
        elseif LS_type == 1 % Find Point satisfying Wolfe conditions
            ...
            if computeHessian
                [t,f,g,LSfunEvals,H] = WolfeLineSearch2(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,LS_saveHessianComp,funObj,updateFuncObj, updateFuncParams, varargin{:});
            else
                [t,f,g,LSfunEvals] = WolfeLineSearch2(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,25,progTol,debug,doPlot,1,funObj,updateFuncObj, updateFuncParams,varargin{:});
            end
            funEvals = funEvals + LSfunEvals;
            %x = x + t*d;
            x = updateFuncObj(x, t, d, updateFuncParams);
            ...
            if method >= NEWTON
                [f_new,g_new,H] = funObj(updateFuncObj(x, t, d, updateFuncParams),varargin{:});
                funEvals = funEvals + 1;
            end
            %x = x + t*d;
            x = updateFuncObj(x, t, d, updateFuncParams);
